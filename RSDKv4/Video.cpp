...
void UpdateVideoFrame()
{
    if (videoPlaying == 2) {
        if (currentVideoFrame < videoFrameCount) {
            ...
            byte fileBuffer[4];
            ushort fileBuffer2[0]  = 0;
            FileRead(fileBuffer, 1);
            videoFilePos += fileBuffer[0];
            FileRead(fileBuffer, 1);
            videoFilePos += fileBuffer[0] << 8;
            FileRead(fileBuffer, 1);
            videoFilePos += fileBuffer[0] << 16;
            FileRead(fileBuffer, 1);
            videoFilePos += fileBuffer[0] << 24;
...
void SetupVideoBuffer(int width, int height)
{
    ...
    if (!Engine.videoBuffer)
        PrintLog("Failed to create video buffer!");
#elif RETRO_USING_SDL2
    Engine.videoBuffer = SDL_CreateTexture(Engine.renderer, SDL_PIXELFORMAT_YV12, SDL_TEXTUREACCESS_STREAMING, SCREEN_XSIZE, SCREEN_YSIZE);
